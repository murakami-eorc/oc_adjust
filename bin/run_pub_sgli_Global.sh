...
#
set progfile="pub_sgli_v208_Global.f"
#
#------------------------------------- HDF5-env
set cpl = `ls -t1 ./pub_sgli_Global_8* | head -1 | tail -c2`
if ( $1 == "i" || $1 == "-i" || ( $1 != "0" && $1 != "-1" && $cpl == "i" ) ) then
 set binfile="pub_sgli_Global_8i"
 ...
 set binfile="pub_sgli_Global_8p"
...
#
if ($1 == "") then
 echo "> run_pub_sgli_Global.sh vnfile swfile outdir(or outfile)"
 exit 19
else if ($1 == "0") then
 pgf95 -O3 -Bstatic $srcdir$progfile -o $bindir$binfile \
       -I$HDF_INC -L$HDF_LIB -lhdf5_fortran -lhdf5 -lz\
       -I$SZPATH/include -L$SZPATH/lib \
       -I$ZPATH/include -L$ZPATH/lib
 exit 20
else if ($1 == "-1") then
 pgf95 -g -Ktrap=fp -C -Bstatic $srcdir$progfile -o $bindir$binfile \
       -I$HDF_INC -L$HDF_LIB -lhdf5_fortran -lhdf5 -lz\
       -I$SZPATH/include -L$SZPATH/lib \
       -I$ZPATH/include -L$ZPATH/lib
 exit 20
else if ($1 == "i") then
 ifort -O3 -Bstatic -assume bscc -assume byterecl -ftz -ip -ipo $srcdir$progfile -o $bindir$binfile\
       -I$HDF_INC -L$HDF_LIB -lhdf5_fortran -lhdf5 -lz \
       -I$SZPATH/include -L$SZPATH/lib\
       -I$ZPATH/include -L$ZPATH/lib
 exit 20
else if ($1 == "-i") then
 ifort -g -assume byterecl -check all -warn all $srcdir$progfile -o $bindir$binfile\
       -I$HDF_INC -L$HDF_LIB -lhdf5_fortran -lhdf5 -lz \
       -I$SZPATH/include -L$SZPATH/lib\
       -I$ZPATH/include -L$ZPATH/lib
 exit 20
else
 echo "% run_pub_sgli_Global.sh "$1 $2 $3
